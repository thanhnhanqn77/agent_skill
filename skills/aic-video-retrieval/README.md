...
| `agents/openai.yaml` | Tên hiển thị và prompt mặc định |
...
GPT-6-Astra chạy trong Codex và gọi script bằng công cụ shell. Chọn GPT-6-Astra ở bộ chọn model của ứng dụng, hoặc dùng CLI bên dưới. Model hỗ trợ image input/function calling; tài khoản cần được cấp quyền model này. Tên model chính xác là `gpt-6-astra`. [Tài liệu model chính thức](https://developers.openai.com/api/docs/models/gpt-6-astra).

**VQA:** AIC_BE hiện không có route sinh đáp án. Agent dùng semantic/OCR/ASR tìm cảnh rồi xem ảnh để trả lời. **Conversation:** dùng `/search_cir` thêm/bớt khái niệm từ một ảnh anchor. Model truy xuất `siglip` trong payload và GPT-6-Astra điều phối là hai cấu hình khác nhau.

## 2. Khởi động agent trong Codex

Mở workspace `D:\AIC\BE`. Codex đọc skill local tại `.agents/skills`; nếu skill mới chưa hiện thì mở phiên mới/khởi động lại. [Hướng dẫn skill chính thức](https://learn.chatgpt.com/docs/build-skills).
