...
description: Nhận query AIC nhập trực tiếp trong hội thoại, tìm video và trả lời KIS, Q&A, TRAKE bằng semantic, temporal, OCR, ASR, conversation/CIR và quan sát ảnh; xuất CSV khi được yêu cầu.
---

# AIC video retrieval

Dùng backend AIC_BE để tìm bằng chứng trong bộ video và trả lời bằng tiếng Việt, trừ khi người dùng yêu cầu ngôn ngữ khác. GPT-6-Astra là agent điều phối và đọc ảnh; `model` trong request AIC_BE là encoder truy xuất, thường là `siglip`, không phải tên GPT.

## Khởi tạo phiên và giới hạn 150 giây

Khi người dùng nói **“Khởi tạo phiên AIC”**, đọc skill này, [session-context.md](references/session-context.md) và [api-contract.md](references/api-contract.md) một lần; thực hiện kiểm tra khởi tạo trong session-context rồi báo sẵn sàng/chỗ chưa truy cập được và chờ query. Không tự giải query cũ hay ví dụ. Giữ cấu hình và trạng thái kiểm tra trong phiên; không đọc lại toàn bộ tài liệu, dò repo hay chạy doctor cho mỗi query. Nếu người dùng gửi query ngay trong phiên chưa khởi tạo, vẫn bắt đầu đồng hồ trước khi chuẩn bị; khởi tạo tối thiểu nằm trong cùng ngân sách.
